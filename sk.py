...
<body>
    ...
    <main>
        ...
        <div class="pagination">
            {% if page|default(1) > 1 %}
                <a href="?page={{ page|default(1) - 1 }}" class="page-link">Назад</a>
            {% endif %}

            {% for p in range(1, total_pages + 1) %}
                <a href="?page={{ p }}" class="page-link {% if p == page|default(1) %}active{% endif %}">{{ p }}</a>
            {% endfor %}

            {% if page|default(1) < total_pages %}
                <a href="?page={{ page|default(1) + 1 }}" class="page-link">Вперед</a>
            {% endif %}
        </div>
