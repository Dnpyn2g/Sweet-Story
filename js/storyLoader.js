        ...
        fetch('stories.json')
            ...
            .then(stories => {
                ...
                if (story) {
                    ...
                    // Разделяем текст истории для добавления рекламы
                    const storyParts = story.content.split(' ');
                    const midpoint = Math.floor(storyParts.length / 2);

                    // Создаём HTML-код с рекламным блоком в середине текста
                    const contentWithAd = `
                        ${storyParts.slice(0, midpoint).join(' ')}
                        <div id="bn_c3a2ba8f5b"></div>
                        <script>
                            ...
                            15),w=(new Date).getTime(),p,H=!("IntersectionObserver"in C),q,d,c,l,A;e?"scroll"==r?b.addEventListener("scroll",t):"lazy"==r?H?f():"loading"==b.readyState?b.addEventListener("DOMContentLoaded",u):u():f():"loading"==b.readyState?b.addEventListener("DOMContentLoaded",f):g("!bn")}catch(a){B(a)}})(window,document,"c3a2ba8f5b","{LOADTYPE}");
                        </script>
                        ${storyParts.slice(midpoint).join(' ')}
                    `;

                    // Устанавливаем контент с рекламой
                    storyContent.innerHTML = contentWithAd;
