...
BASE_DIR = r"C:\Users\dnpyn\Desktop\optimistka"
STORY_FOLDER = os.path.join(BASE_DIR, "stories")  # Основная папка с историями
STORY_JSON_PATH = os.path.join(STORY_FOLDER, "stories.json")  # Файл с историями
IMAGE_STORAGE1 = STORY_FOLDER  # Первая папка с изображениями
IMAGE_STORAGE2 = os.path.join(BASE_DIR, "sweet-story", "images")  # Вторая папка с изображениями
FAVORITES_FILE = os.path.join(STORY_FOLDER, "favorites.json")  # Файл с избранными историями

# Функция загрузки избранных историй
...
# Маршрут для раздачи изображений (ищет в двух папках)
@app.route("/optimistka/images/<path:filename>")
def serve_story_image(filename):
    path1 = os.path.join(IMAGE_STORAGE1, filename)
    path2 = os.path.join(IMAGE_STORAGE2, filename)

    if os.path.exists(path1):
        return send_from_directory(IMAGE_STORAGE1, filename)
    elif os.path.exists(path2):
        return send_from_directory(IMAGE_STORAGE2, filename)
    else:
        return "Изображение не найдено", 404

# Маршрут для отображения страницы с историями (с пагинацией)
@app.route("/optimistka")
def optimistka():
    try:
        with open(STORY_JSON_PATH, "r", encoding="utf-8") as file:
            stories = json.load(file)
    except Exception as e:
        return f"Ошибка загрузки историй: {str(e)}"
    ...
    stories_on_page = stories[start:end]

    return render_template_string(
        OPTIMISTKA_TEMPLATE,
        stories=stories_on_page,
        total=len(stories),
        favorites=favorites,
        page=page,
...
# Маршрут для добавления/удаления избранных историй
@app.route("/optimistka/favorite", methods=["POST"])
def toggle_favorite():
    data = request.json
    ...
    if story_id in favorites:
        favorites.remove(story_id)
    else:
        favorites.append(story_id)
...
# HTML-шаблон для отображения историй с измененной позицией кнопки "Скрыть текст"
OPTIMISTKA_TEMPLATE = """
<!DOCTYPE html>
<html lang="ru">
<head>
    <meta charset="UTF-8">
    <meta name="viewport" content="width=device-width, initial-scale=1.0">
    <title>Истории «ОПТИМИСТКА»</title>
    <style>
        body { font-family: Arial, sans-serif; background-color: #f4f4f4; text-align: center; margin: 0; padding: 0; }
        .container { max-width: 1000px; margin: 20px auto; padding: 20px; background: white; border-radius: 8px; box-shadow: 0px 0px 10px rgba(0, 0, 0, 0.1); }
        h1 { color: #333; }
        .summary { margin-bottom: 20px; font-size: 1.2em; color: #555; }
        .story { display: flex; align-items: center; gap: 15px; padding: 15px; border-bottom: 1px solid #ddd; flex-direction: column; text-align: left; }
        .story h2 { margin: 10px 0; color: #007BFF; flex: 1; }
        .story img { width: 300px; height: auto; border-radius: 8px; object-fit: cover; }
        .story-content { display: none; margin-top: 10px; }
        .toggle-btn, .favorite-btn, .home-btn, .favorites-btn, .pagination-btn { background-color: #007BFF; color: white; border: none; padding: 8px 12px; cursor: pointer; border-radius: 4px; margin-right: 5px; }
        .favorite-btn { background-color: #FF0000; }
        .favorites-btn { background-color: #28a745; }
        .pagination-btn { background-color: #555; }
...
<body>
    <div class="container">
        <h1>Истории «ОПТИМИСТКА»</h1>
        <p class="summary">Всего историй: {{ total }}</p>
        <button class="home-btn" onclick="window.location.href='/'">На главную</button>
        <button class="favorites-btn" onclick="window.location.href='/optimistka/favorites'">Избранные истории</button>

        {% for story in stories %}
        <div class="story {% if story['id'] in favorites %}favorite{% endif %}" id="story-{{ story['id'] }}">
            <img src="/optimistka/images/{{ story['id'] }}.jpg" alt="Изображение {{ story['title'] }}">
            <div class="story-details">
                <h2>{{ story['title'] }}</h2>
                <p><strong>ID:</strong> {{ story['id'] }}</p>
                <button class="favorite-btn" onclick="toggleFavorite({{ story['id'] }})">Избранное</button>
                <p id="content-{{ story['id'] }}" class="story-content">{{ story['text'] }}</p>
                <button id="button-{{ story['id'] }}" class="toggle-btn" onclick="toggleContent({{ story['id'] }})">Показать текст</button>
            </div>
        </div>
        {% endfor %}
        ...
        <div style="margin-top: 20px;">
            {% if page > 1 %}
                <a href="?page={{ page - 1 }}">
                    <button class="pagination-btn">← Предыдущая</button>
                </a>
            {% endif %}
